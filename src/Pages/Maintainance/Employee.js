  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      if (edit) {
        ...
        await employeeAPI.updateData(editCode, newData, user).then(() => {
      ...
      //add the new data
        ...
        if (obj === undefined) {
          await employeeAPI.createData(newData, user).then(() => {
  ...
  const handleDelete = async () => {
    if (editCode !== "choose") {
      const res = await employeeAPI
        .deleteData(editCode, user)
        .then((res) =>
          handleAlert({ type: "success", text: `Code ${editCode} deleted successfully` })
        );
  ...
  return (
    <div className="mx-auto">
      ...
      <div className="border border-secondary mt-1">
        ...
        <button className="border-0 border-end border-secondary p-3" onClick={handleDelete}>
          <i className="material-icons text-danger fs-5">delete</i> <br /> Delete
        </button>
