...
const CashGood = ({ transactionType }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const res1 = await consigneeAPI.readData(user);
      const res2 = await elementAPI.readData(user);
  ...
  const handleSelection = (e) => {
    if (e.target.value !== "choose") {
      if (e.target.name == "consigneeCode") setConsigneeCode(e.target.value);
      else setElementCode(e.target.value);
    } else {
      if (e.target.name == "consigneeCode") setConsigneeCode("choose");
      else setElementCode("choose");
    }
  };
  ...
  const handleSubmit = async () => {
    if (lineItems.length === 0 || consigneeCode === "choose") {
      if (lineItems.length === 0 && consigneeCode === "choose")
        handleAlert({
          type: "danger",
          text: `Make sure to add line item and to choose consignee code`,
        });
      else if (lineItems.length === 0)
        handleAlert({ type: "danger", text: `Make sure to add line item` });
      else handleAlert({ type: "danger", text: `Make sure to choose consignee code` });
    } else {
      const newData = {
        remark: "",
      ...
      try {
        await voucherAPI.createData(newData, user);
  ...
  const getGrandTotal = () => {
    let tax = lineItems.reduce(
      (total, current) =>
        total +
        (current.quantity * current.unitAmount + current.quantity * current.unitAmount * 0.15),
      0
    );
    return tax;
  };
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="border border-secondary mt-1 mb-2">
        <button className="border-0 border-end border-secondary p-3" onClick={handleNew}>
          <i className="material-icons fs-6">autorenew</i> <br /> New{" "}
        </button>

        <button className="border-0 border-end border-secondary p-3" onClick={handleSubmit}>
          <i className="fa fa-save fs-5 text-primary"></i> <br /> Save{" "}
        </button>
      </div>
          ...
          >
            ...
            {consigneeDatas &&
              consigneeDatas.map((data, next) => (
                <option key={next} value={data.code}>
                  {" "}
                  {data.code}{" "}
                </option>
              ))}
      ...
      <div className="d-md-flex align-content-center justify-content-between border mt-3">
        ...
        <div>
          ...
          <input
            type="text"
            name="uom"
            value={elementCode !== "choose" ? getUom(elementCode) : ""}
            readOnly
            placeholder="UOM"
            style={{ height: 40 }}
          />
          ...
          {lineItems &&
            lineItems.map((value, index) => (
              <tr key={index}>
                ...
                <td>
                  {value.quantity * value.unitAmount + value.quantity * value.unitAmount * 0.15}
                </td>
              </tr>
            ))}
        </tbody>
